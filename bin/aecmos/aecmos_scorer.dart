...
//
// onnx_runtime_dart is a dev_dependency on purpose (this headless eval tool must
// never reach the app / web build — its `_io` loader uses dart:io); bin/ code
// using a dev-dependency trips depend_on_referenced_packages, which is the
// intended boundary here, so it is suppressed for this file.
...
class AecmosScorer {
  ...
  Float32List transform(Float32List signal) => aecmosMelFeatures(
        signal,
        sr: samplingRate,
        nFft: dftSize + 1,
        hopLength: dftSize ~/ 2,
