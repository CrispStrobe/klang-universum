// High or Low? — the pitch-direction sort. Drives real drag gestures: each note
// only drops into its correct high/low basket, and sorting all four advances the
...
/// its correct high/low basket, so try both).
Future<void> _sortAllCards(WidgetTester tester) async {
  ...
  for (var placed = 0; placed < PitchSortScreen.cardCount; placed++) {
    for (var b = 0; b < 2; b++) {
...
void main() {
  ...
  testWidgets('sorting every note into its basket advances the round',
    ...
    await pumpGame(tester, const PitchSortScreen(), sri: sri);

    expect(find.text('Round 1 of 6'), findsOneWidget);
    expect(_cards(), findsNWidgets(PitchSortScreen.cardCount));
    ...
    // Correct drops score into the pitch-height skill.
    expect(sri.getDetailedBreakdown()['pitch']!.keys, ['height']);
  });

  testWidgets('the bass variant reads in the bass clef and still sorts',
      (tester) async {
    final sri = SriService(getNow: () => DateTime(2026, 7, 11));
    await pumpGame(tester, const PitchSortScreen(clef: Clef.bass), sri: sri);
