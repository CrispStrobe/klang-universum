...
void main() {
  ...
  testWidgets('tapping a clip opens the inspector; gain slider changes gain',
    ...
    // in the body, the inspector adds 7 sliders (gain, pan, width, 2 fades, 2 trims); the
    // modal renders after the body, so those are the LAST 7.
    await tester.tap(find.text('🥁'));
    await tester.pumpAndSettle();
    final sliders = find.byType(Slider);
    final total = tester.widgetList(sliders).length;
    expect(total, greaterThanOrEqualTo(7));

    // Drag the gain slider (first of the inspector's 7) down; gain drops.
    await tester.drag(sliders.at(total - 7), const Offset(-80, 0));
    await tester.pump();
