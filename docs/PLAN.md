...
- **opus (loop-juice)** · 🚧 **ACTIVE — Loop Mixer 3.0 §E-1: secret combos.** Certain enabled built-in sets unlock a named combo → a reveal + a "found N/M" counter, turning the sandbox into a discovery game. Pure core `lib/features/games/composition/loop_secrets.dart` (`matchCombo`, unit-tested) + screen wiring in `loop_mixer_screen.dart` (reveal snackbar + counter, checked after toggle/roll) + EN/DE. No `loop_engine.dart`; no overlap with loop-mixer-3.
