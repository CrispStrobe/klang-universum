...
- **opus (loop-sheet-fix)** · 🚧 **ACTIVE — Loop Mixer §A: fix the broken "show as sheet music" live-engraving panel.** Files: `lib/features/games/composition/loop_mixer_screen.dart` (the score panel ~:1362) + maybe `groove_notation.dart` + a new widget/layout test. `grooveScore` is pure + unit-tested, so diagnosing the app path (StaffView API drift / layout render / progression-mode chord cells). NOT touching `loop_engine.dart` or the DAW files (daw-workshop agent's territory).
