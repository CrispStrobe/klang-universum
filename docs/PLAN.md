...
- **opus (connect-roadmap)** · 🚧 **ACTIVE — new minigame: Connect the Road Signs** (`ConnectMode.navigation` — match each navigation term Da Capo/Dal Segno/Fine/Coda/Segno/al Fine/al Coda to what it tells you to do). Fills the uncovered roadmap/repeat-sign reading skill; terms are universal Italian (hardcoded like tempo mode, no exotic glyphs), only meanings localised. Files: `connect_line_screen.dart` (enum case + item builder) + `GameInfo` + `concept_map` (song_form, "…repeats") + EN/DE + widget test. Low-collision additive (NB: the song_form concept-list line may also be edited by gap-games — trivial append merge if so).
