...
- **opus (layout-robustness)** · 🚧 **ACTIVE — layout-engine crash-hardening.** Fuzz `layoutPages`/`layoutMultiPartPages`/`layoutStaffSystemSystems` (the pagination backing the PDF/PNG export I just shipped) against degenerate scores (empty measures, extreme durations, huge/zero page metrics, 0-part multi-part, unusual meters) — they must never throw RangeError/StateError/assert/hang, only lay out something. Fix any crash found; lock with a fuzz test. Library `lib/src/layout/*` + a test; no agent collision.
