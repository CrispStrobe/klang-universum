...
- **opus (connect-keysig)** · 🚧 **ACTIVE — new minigame: Connect the Key Signatures** (`ConnectMode.keySignatures` — match a rendered key signature to how many accidentals it has, e.g. "2 sharps"). Distinct from Key Quiz (count, not name → dodges the B/H German-naming issue); thickens the thin `key_sig` concept. Renders key-sig cards via `Score.simple(keySignature:, notes: "r:w")` + StaffView (reuses the notes-mode card pattern). Files: `connect_line_screen.dart` (enum case + item builder) + `GameInfo` + `concept_map` + EN/DE (ICU plural) + widget test. Low-collision additive.
...
- **opus (mp3-import)** · ✅ **SHIPPED — import MP3 audio as samples/clips + sample packs (uses our pure-Dart `mp3Decode`).** Audio import was WAV-only; now a **Flutter-free** shared `lib/shared/music_io/audio_import.dart` — `importAudioMono(bytes)` **magic-byte**-detects WAV vs MP3 (not extension) → mono float + sample rate (MP3 via our all-block-type `mp3Decode`, stereo averaged), + `kAudioImportExtensions` (`['wav','mp3']`, a plain list so the decoder has no `file_selector` dep) + `ImportedAudio`. Wired into **(1) Voice Lab** (picker takes WAV+MP3; "Load WAV"→"Load audio"/"Audio laden" EN+DE) and **(2) the sample-pack extractor** (`sample_extractor.dart` — a zip/7z of **MP3** loops/one-shots now extracts alongside WAV, reusing the pure helper so the extractor stays Flutter-free). +8 unit tests (WAV/MP3/stereo round-trips, junk/empty→null, MP3-in-pack); full-project analyze clean; import/sample-extractor/voice-lab suites green (29). Files: new `audio_import.dart` + `voice_lab_screen.dart` + `sample_extractor.dart` + one ARB label. Now idle. **Next (unclaimed):** the tracker WAV-sample picker (`advanced_tracker_screen.dart:2011`, held by tracker-adv) is the last WAV-only audio-import site.
