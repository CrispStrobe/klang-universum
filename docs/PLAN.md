...
- **opus (loop-juice)** · 🚧 **ACTIVE — Loop Mixer 3.0 §D-2: shape-creatures (maintainer: shape-creatures w/ musical theme).** Each track card gets a little animated shape-creature performer (drum/bass/keys/note/star/mic/bars emblem per instrument; awake+happy when enabled, asleep when off), inheriting the card beat-pulse. New `loop_creatures.dart` (pure `creatureShapeFor` mapping + `LoopCreature` CustomPaint) + wire into `_TrackCard` (replace the icon). Screen-only, card region — no `loop_engine.dart`, no overlap with loop-mixer-3. Next: §C-1 streaming filter (engine).
...
- **opus (loop-mixer-3efg)** · 🚧 **ACTIVE — Loop Mixer 3.0 §C–§G (maintainer: do them all), the buildable+testable subset.** Worktree `../mus-textbook`, branch `feature/textbook-prose-anavis`. Note: §C-4 dice + §G-3 save-slots already shipped by other agents. Building, each with unit+widget tests: **§C-2 one-knob master filter** (offline low-pass↔high-pass sweep on the mix bus via `biquadFx`, seam-continuous like the sends; ephemeral live control), **§C-3 quantized launch** (card toggles queue to the next bar with an armed pulse; the seam scheduler already swaps at the boundary), **§E-1 secret combos** (a data table of enabled-stem sets → a bonus + a per-style found-N tracker), **§E-2 band challenges** (zero-pressure prompts). Art/DSP-blocked items (§D-2 characters, §C-1 streaming effect strip, §F/§G-1/§G-2) flagged for a session with audio/art review. Touching `loop_engine.dart` + `loop_mixer_screen.dart` + ARBs (no active collision).
