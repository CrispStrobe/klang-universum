...
- **opus (connect-tenor)** · 🚧 **ACTIVE — new minigame variant: Connect the Notes — Tenor clef** (`connect_line_tenor`, `ConnectLineScreen(clef: Clef.tenor)`). Thickens the thin `note_reading_tenor` concept with a matching-modality drill; the blessed "cheap depth: clef variant" pattern. Also fixes a latent bug: the connect `progressId` only special-cased bass, so a non-treble/bass clef would collide with treble progress. Files: `connect_line_screen.dart` (progressId case) + one `GameInfo` + `concept_map` + a widget test (own progressId). Low-collision additive.
