...
- **opus (loop-juice)** · 🚧 **ACTIVE — Loop Mixer 3.0 §C-4: dice / "surprise me".** A dice button in the top control row that rolls a fresh always-good groove (random enabled set: drums anchor + a random mix guaranteeing ≥1 melodic voice, random variant per track, a gentle swing roll) — every combo is consonant so any roll works. Pure UI + existing engine API in `loop_mixer_screen.dart` (a `_roll()` + one `IconButton` + EN/DE key). No `loop_engine.dart`; localized to the control row — no overlap with loop-mixer-3.
...
- **opus (loop-mixer-3b)** · 🚧 **ACTIVE — Loop Mixer 3.0 §B item 2: swappable drum kits.** Worktree `../mus-textbook`, branch `feature/textbook-prose-anavis`. Parameterizing `renderDrum` (`synth.dart`) with a `DrumKit` profile (tune / decay / noise / pitch-sweep depth / lo-fi crush), threading `kit` through `DrumRowsPattern.render` → every engine drum render site (vamp, tiled, varied, fill), adding `GrooveSpec.kitId` (token `kt`, omitted at default so old `KU1.` decode) + engine `kit` setter (clears render caches), a kit chip row in `loop_mixer_screen.dart`, + EN/DE keys. Pure timbre — the onset grid is invariant. Verify headlessly: two kits render different spectra/decay but identical hit positions; token roundtrip; the fill path also swaps kit. Touching `synth.dart` + `loop_engine.dart` + `loop_mixer_screen.dart` + ARBs (no active collision — loop-sheet-fix idle).
