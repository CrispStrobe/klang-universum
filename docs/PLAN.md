...
- **opus (loop-juice)** · 🚧 **ACTIVE — Loop Mixer 3.0 §D-1: beat-reactive cards.** Make the track cards pulse/glow on the beat (stronger on the downbeat) so the surface visibly breathes with the music — the biggest fix for "static form" feel. Pure UI in `lib/features/games/composition/loop_mixer_screen.dart` (a `_BeatPulse` wrapper around `_TrackCard`, driven by the existing `_step` beat notifier); NO engine change, NO `loop_engine.dart`. Localized to the cards — no overlap with loop-mixer-3's key/scale chips (harmony area) or its remaining `engravedCellsFor` swaps.
...
- **opus (loop-mixer-3)** · 🚧 **ACTIVE — Loop Mixer 3.0 §B item 1: key & scale UI (engine already shipped `897b246`; §A now landed so the screen is free).** Now adding the two chip rows (key C…B + Major/Minor) to `loop_mixer_screen.dart` wired to `engine.key`/`engine.scale` via `_syncPlayback`, + tester seams + a widget test, + pointing the follow-engine (`:480`) and Song-Book export (`groove_notation.dart:151`) at `engravedCellsFor`, + EN/DE keys. Score-panel engraving already uses `engravedCellsFor` (loop-sheet-fix `86c0930`). Touching `loop_mixer_screen.dart` + `groove_notation.dart` + ARBs (loop-sheet-fix is idle/shipped — no live collision). Worktree `../mus-textbook`, branch `feature/textbook-prose-anavis`. ✅ **Engine layer SHIPPED (`897b246`, +10 tests, `loop_engine.dart` only — loop-sheet-fix explicitly not touching it, no collision):** `GrooveSpec.key` (root 0–11) + `scale` (major/minor pentatonic); backward-compatible token (`k`/`sc` omitted at defaults so old `KU1.` tokens decode byte-identically; hostile key wrapped to 0–11); `pitchTranspose = key + (minor?3:0)`; a `transpose` param (default 0) threaded through `renderCells`/`LoopPattern.render` + every pitched render path; `engravedCellsFor` (transposed cells for engraving/jam/export); `jamFit` scale+chord sets shifted by the root. Minor pentatonic borrows the relative-major set (+3) so any key×scale is a RIGID transposition of the authored C-pentatonic content → stays consonant. Verified: a real detector reads renderCells C4→F4 under transpose; rigid-transposition invariant across every key×scale×track; token roundtrip/backward-compat. Existing loop suites green (transpose defaults to 0). ⏸️ **DEFERRED follow-up (waiting on loop-sheet-fix to land §A in `loop_mixer_screen.dart`):** two chip rows (key C…B + major/minor) wired to `engine.key`/`engine.scale`, the `:1372` engraving swap to `engravedCellsFor`, the follow-engine + Song-Book export to `engravedCellsFor`, + EN/DE keys. **loop-mixer-3 IDLE until §A ships.**
