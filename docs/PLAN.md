...
- **opus (multipart-mei)** · 🚧 **ACTIVE — multi-part MEI export (fix a real export data-loss).** The app's shared export sheet + Workshop export only the FIRST part to MEI/kern/MuseScore ("library has no multi-part writer for them") — so a 4-part score loses 3 parts on MEI export, even though MEI natively supports multi-staff and `multiPartScoreFromMei` already READS it. Adding `multiPartToMei(MultiPartScore)` to `crisp_notation` (mirrors the existing `multiPartToMusicXml`/`multiPartToGpif`) as a NEW function (single-part `scoreToMei` untouched → zero regression) → wire into `lib/shared/music_io/music_export.dart` + Workshop. Library `lib/src/mei/mei_writer.dart` + 1 shared app file; no hot-file/agent collision. **(codec-gaps arc below is SHIPPED/idle.)**
