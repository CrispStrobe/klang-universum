...
**`AnalysisHubScreen`** ("See the Music", `analysis_view` tile) hosting both. Still
open (if ever wanted): T/S/D **span colouring over a single multi-chord score**
(vs the current per-chord blocks) and cadence-point markers on the staff.
