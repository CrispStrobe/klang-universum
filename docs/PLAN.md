...
- **opus (loop-juice)** · 🚧 **ACTIVE — Loop Mixer 3.0 §C-1b: streaming effect DSP core.** The buildable+testable slice of the full streaming FX path (backend §C-1a + UI §C-1c scoped, awaiting the audio-backend commitment). New pure `lib/core/audio/streaming_filter.dart`: a stateful seam-continuous bipolar LP↔HP `StreamingFilter` (own Direct-Form-I RBJ coeffs, live-tunable cutoff) + unit tests (LP/HP attenuation, one-block==two-block continuity, swept-cutoff bounded). Flutter-free, new file — NO collision with loop-mixer-3efg (their §C-2 is offline `biquadFx` on the render).
