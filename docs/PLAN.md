...
- **opus (loop-mixer-3)** · 🚧 **ACTIVE — Loop Mixer 3.0 §B item 1: key & scale (maintainer's chosen lead).** Worktree `../mus-textbook`, branch `feature/textbook-prose-anavis`. **Engine-only this slice** (`loop_engine.dart` — which loop-sheet-fix explicitly is NOT touching, so no collision): `GrooveSpec.key` (root 0–11) + `scale` (major/minor pentatonic), backward-compatible token (`k`/`sc` omitted at default so old `KU1.` decode), a `transpose` param threaded through `renderCells`/`LoopPattern.render` (defaults 0), `engravedCellsFor` (transposed cells for engraving/jam), and `jamFit` scale/chord set transposed by the root (minor = relative-major set, +3 → stays consonant). Verify headlessly: render → `listen.dart` reads transposed notes + token roundtrip. **DEFERRED to a follow-up** (after loop-sheet-fix lands §A): the two chip rows + the `:1372` `engravedCellsFor` swap in `loop_mixer_screen.dart` (their active file) + EN/DE keys.
