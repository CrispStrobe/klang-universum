...
- **opus (codec-gaps)** · 🚧 **ACTIVE — closing the remaining crisp_notation codec round-trip gaps** the sweep documented, one at a time (writer+reader → probe → flip the `roundtrip_features_test` matrix cell → ship to public `crisp_notation@main`). Library only (`lib/src/{mei,humdrum,abc}/*` + test files), no app hot files. **✅ ALL MEI GAPS CLOSED:** dynamics (`<dynam>`, `2c9011b`), repeats+voltas (`left/right` + `<ending>`, `32c17c7`), navigation (`<repeatMark>`, `5abfb69`), lyrics (`<verse>/<syl>`, `5f2f82b`) — plus ornaments earlier (`d688a43`). **✅ kern repeats** (barline `:|`/`|:` signs, `c0176ff`). **Remaining:** kern dynamics + lyrics (need parallel `**dynam`/`**text` spines — confirmed **zero-regression** via a conditional spine since the 150-score property test adds neither), kern voltas (`*>` section labels) + navigation (no standard Humdrum form), and tremolo (ABC/MEI/kern). **(CI-fixes work below is SHIPPED/idle.)**
