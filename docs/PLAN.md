...
- **opus (loop-juice)** · 🚧 **ACTIVE — Loop Mixer 3.0 §G-3: save slots.** "Keep your bands": a named list of groove share tokens in SharedPreferences. Pure `GrooveSlotsService` (`groove_slots.dart`, unit-tested) + share-sheet entries (Save / My grooves…) in `loop_mixer_screen.dart` + test seams + EN/DE. No `loop_engine.dart`; no overlap with loop-mixer-3.
