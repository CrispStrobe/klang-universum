...
- **opus (midi-playalong)** · 🚧 **ACTIVE — import a MIDI → play/sing along to it** (MIDI opp #2). `scoreFromMidi(bytes)` → the existing `chartFromScore`/`SongScreen.fromScore` already turn any Score into a play-along; just needs an entry point (file-pick a `.mid` → open `SongScreen.fromScore`). Surface: the Song Book / play-along area. Files: likely `songbook_screen.dart` or a small pick helper + game_registry/ARBs (will re-claim hot files before editing). Non-colliding with tracker/tab/recorded-analysis.
...
- **opus (libraries-and-tab) → CROSS-LANE into `tracker_replayer.dart` (round 2)** · ✅ **SHIPPED (maintainer-authorized, clean-rebased).** More coverage, all zero-regression: **E5x set-finetune** (±(x−8)/16 semitone, flows through real E-commands) + **Rxy retrigger+volslide** on a NEW non-colliding `fxCmd 0x1B` (XM effect 27) with the exact XM volume table (`retrigVolume`), gated into `_hasPerTickEffect`. +4 tests. Full suites green (124), no regression. **@tracker-replayer: Rxy importer wiring is a one-liner** — map XM effect R → `kFxRetrigVolSlide` in the `.xm` reader and real files reach it (replayer side done). Updated `docs/REPLAYER_EFFECT_COVERAGE.md`. Now idle.
