...
- **opus (connect-progress-lock)** · 🚧 **ACTIVE — lock the connect-family progressId isolation.** Guards the exact bug class behind the tenor-clef fix (a ConnectMode/clef silently sharing another games progress). Add a test in `connect_line_test.dart`: every registered connect `(mode, clef)` produces a UNIQUE, non-empty progressId, and the set of produced progressIds equals the set of `connect_*` game ids in the registry (so a new mode/registry entry cant drift out of sync). Test-only, zero collision.
