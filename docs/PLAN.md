...
- **opus (groove-token-fuzz)** · 🚧 **ACTIVE — harden the `KU1.` groove share token (the app's one untrusted-input surface).** Worktree `../mus-textbook`, branch `feature/textbook-prose-anavis`. A property/fuzz test in a NEW `test/groove_token_fuzz_test.dart`: (a) round-trip fidelity — random valid `GrooveSpec`s survive encode→decode by `cacheKey`; (b) adversarial resilience — random garbage + `KU1.`+random-base64 + hand-built wrong-type/extreme JSON never make `decodeGrooveToken` throw, and any decoded spec `applySpec`+`renderLoop`+`renderVariedLoop` without throwing. Fix any crash found. Test-only + (if needed) `loop_engine.dart` defensive fixes — uncontested (no active agent on the loop engine), NO hot files.
