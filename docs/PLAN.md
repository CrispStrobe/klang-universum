...
- **opus (smear-capture)** · 🚧 **ACTIVE — §F-1 follow-up: capture the smear-pad improvisation into a keepable layer.** Worktree `../mus-textbook`, branch `feature/textbook-prose-anavis`. Record timed `(phaseMs, midi)` samples as the finger drags the solo pad → on "Keep", reuse `groove_capture.quantizeToGroove` (→ pentatonic-snapped grid cells) → `engine.setUserTrack` so the improvised lead joins the band as the sung-voice card. Pure/testable (the quantize is already unit-tested; new bits are the timed recording + the keep path). Touching `loop_mixer_screen.dart` (mine — the smear pad) + ARBs. No collision.
