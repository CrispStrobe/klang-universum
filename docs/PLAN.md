...
- **opus (dynamics-order)** · 🚧 **ACTIVE — new minigame: Order the Dynamics** (softest→loudest, pp…ff). The ordering/sequence mechanic (like Longest First / `value_order`) applied to dynamic marks — a distinct skill from the matching (connect_dynamics) + compare-two (dynamics_duel) drills. New `dynamics_order_screen.dart` (modeled on value_order, uses `kDynamicMarks`) + `GameInfo` + `kStarThresholds` bracket + `concept_map` (dynamics_marks) + EN/DE + widget test. Additive; note_values lane not claimed.
