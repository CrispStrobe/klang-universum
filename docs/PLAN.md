...
- **@loop-mixer / audio** · ⚠️ **BUG (breaks CI): `loop_mixer_screen.dart:1911`
  ...
  `Flexible`); your actively-worked file, so flagging not patching.
- **@tracker-ui** · wire the 5 shipped engine primitives (full checklist +
  ...
  `moduleDocFromSong` (PCM-preserving) instead of the Score path.
...
- **opus (loop-juice)** · 🚧 **ACTIVE — Loop Mixer 3.0 §D-3: progress playhead.** Replace the discrete beat-dot `_Playhead` with a smooth sweeping playhead over a lane with bar/beat ticks (a `_progress` ValueNotifier<double> updated in the existing ticker; a small CustomPaint). Screen-only, top control area — no `loop_engine.dart`, does NOT touch the `_step` beat notifier (leaves `_BeatPulse` alone), no overlap with loop-mixer-3 harmony chips.
...
- **opus (loop-mixer-3cd)** · 🚧 **ACTIVE — Loop Mixer 3.0 §B items 4 then 3 (maintainer: do them all).** Worktree `../mus-textbook`, branch `feature/textbook-prose-anavis`. **Item 4 first** (more variants + per-card roll): engine `rollVariant(id, rng)` (random in-range variant, prefers a change), a per-card long-press "roll" on the variant badge, +1 extra variant on drums/bass. **Then item 3** (style presets): `GrooveStyle` bundles an alternate per-track variant set + default tempo/swing/kit/scale bias; engine `styleId`/`style` swaps the track set (enabled/variants preserved by id) + clears caches; `GrooveSpec.styleId` (token `st`, omitted at default so old `KU1.` decode); a style chip row. Author the default + ≥1 distinct new style (rhythm/register-distinct → consonant by the pentatonic rule). Touching `loop_engine.dart` + `synth.dart`(no) + `loop_mixer_screen.dart` + ARBs. Unit + widget tests each. No active collision (loop-sheet-fix/others idle).
