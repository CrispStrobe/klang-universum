...
- **opus (score-invariants)** · 🚧 **ACTIVE — lock the core `scoreToStars` contract with property tests.** The star-scoring used by all 142 games has only a per-game hit-% test; add `test/score_to_stars_test.dart` asserting the invariants across every registered gameType + unknowns + a fuzz: result always in 0-3, lost game → always 0, successful → monotonic non-decreasing in score, each threshold bracket produces the right star at its boundary, the fallback (800/400) for unknown types. Test-only, new file — zero collision.
...
- **opus (mp3-import)** · ✅ **SHIPPED — import MP3 audio files as samples/clips (uses our pure-Dart `mp3Decode`).** Audio import was WAV-only; now a shared `lib/shared/music_io/audio_import.dart` — `importAudioMono(bytes)` **magic-byte**-detects WAV vs MP3 (not extension) → mono float + sample rate (MP3 via our all-block-type `mp3Decode`, stereo averaged to mono), plus `kAudioImportGroup` (`['wav','mp3']`) and an `ImportedAudio` result. Wired into **Voice Lab** (`voice_lab_screen.dart` picker now takes WAV+MP3; label "Load WAV"→"Load audio"/"Audio laden", EN+DE). +6 unit tests (WAV+MP3+stereo round-trips, junk→null, empty→null, type group); full-project analyze clean; voice_lab/sound_lab/import/export suites green (27). Only a NEW shared file + `voice_lab_screen.dart` + the one ARB label. Now idle. **Next (unclaimed):** the same helper drops into other import sites (`sample_extractor`, any "add sample" picker) for MP3 support.
...
- **opus (detector-hardening)** · 🚧 **ACTIVE — robustness-harden the live mic pitch detector (`pitch_analysis.dart`).** Worktree `../mus-textbook`, branch `feature/textbook-prose-anavis`. The Tuner / Play-along / Chord-listener feed it untrusted live frames (silence, DC offset, clipping, or NaN/Inf from a bad plugin frame). **Real fix:** a non-finite input frame makes `rms = sqrt(energy/n)` NaN/Inf, and `NaN < 1e-3` is false so it doesn't bail → the reading leaks a **NaN/Inf `rms`** to consumers (`beat_capture`/`groove_capture` onset thresholds read `reading.rms`). Add a finite-guard (non-finite frame → clean silent reading, rms 0) + a defensive `freq.isFinite` guard, then a robustness test (degenerate/adversarial PCM: empty/tiny windows, all-zero, DC, clipped, NaN/Inf, noise → never throws, every reading field finite). Uncontested (no agent on the detection core), no hot files.
