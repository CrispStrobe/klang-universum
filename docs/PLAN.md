...
- **opus (loop-juice)** · 🚧 **ACTIVE — Loop Mixer 3.0 §C-1a (testable core): streaming mixer + sink contract.** The pure, CI-safe portion of the streaming backend: `lib/core/audio/streaming_mixer.dart` — `StreamingAudioSink` interface + `BufferedSink` (test impl) + `StreamingMixer` (pulls the loop PCM through the `StreamingFilter` chain at a wrapping play position, feeds a sink in callback-sized blocks, seam-continuous). Unit-tested. The native platform sink stays OUT of pubspec (CI-safety, per the AEC-plugin rule) — the `StreamingAudioSink` interface IS its integration seam. New file — no collision.
...
- **opus (loop-mixer-3fg)** · 🚧 **ACTIVE — Loop Mixer 3.0 remaining §C–§G, buildable "first cut" of each (maintainer: do them all).** Worktree `../mus-textbook`, branch `feature/textbook-prose-anavis`. Order by testable-logic value: **§G-1 section/scene grid** (4 scene slots snapshot enabled+variants; tap=launch, long-press=capture; a chain toggle auto-advances scenes at the seam — the direct answer to "it's just one loop", composes §C-3), **§F-1 scale-locked smear pad** (drag → in-key pentatonic note; the note-mapping is pure+testable even if gesture feel needs review; captured into a layer), **§D-3 step playhead + §D-1 reactive cards** (per-step stem energy from the engine → card pulse; the energy is testable), **§C-1 first-cut momentary effects** (pre-rendered discrete-intensity offline effects on the mix bus — extends §C-2). Each: unit + widget tests. Art/streaming-DSP-true items stay flagged. Touching `loop_engine.dart` + `loop_mixer_screen.dart` + ARBs (coordinating; peers shipped creatures/secrets/slots/dice).
