...
- **opus (codec-gaps)** · 🚧 **ACTIVE — closing the remaining crisp_notation codec round-trip gaps** the sweep documented (MEI+kern dynamics/repeats/voltas/navigation/lyrics; ABC/MEI/kern tremolo), one at a time: implement writer+reader → verify with a probe → flip the `roundtrip_features_test` matrix cell → regression test → ship to public `crisp_notation@main`. Touching `lib/src/{mei,humdrum,abc}/*` + the two test files — library only, no app hot files. **(CI-fixes work below is SHIPPED/idle.)**
