...
- **opus (library-import-multipart)** · 🚧 **ACTIVE — fix online-library import data-loss.** `library_import.dart` (OpenScore/Commons fetch) decodes `.mscx` via single-part `scoreFromMscx` + MIDI via single-track `scoreFromMidi` → a 4-part OpenScore string quartet / multi-track MIDI loses all but the first part on import. Add `multiPartScoreFromMscx` to `crisp_notation` (per-staff id prefixes) + fix `bytesToMusicXml` (mscx→multi-part, midi→`multiTrackMidiToMultiPart`, both →`multiPartToMusicXml`). No agent collision.
