...
**Neural voice (CrispASR / Kokoro).** Behind the same seam, `core/audio/tts/` adds a higher-quality on-device backend: the `crispasr` pub FFI package → `libcrispasr` (ggml) running **Kokoro** (82 M params, Apache-2.0, de+en). Synthesis runs in a background isolate (→ 24 kHz PCM → WAV → AudioService), the model is **downloaded on first use + cached** (never bundled), and a conditional-import facade keeps dart:io/ffi out of the web build (web → null stub). `TtsService` prefers the neural voice when the native lib + model are present, else the platform voice — so the app always speaks. The real macOS synth path is test-verified; per-platform lib bundling + publishing the Kokoro GGUFs are the remaining native steps.
